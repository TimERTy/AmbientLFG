...
- The role checkboxes are gone. Which seats you can take is Blizzard's own "Tank/Healer/Damage role available" filter, and the window now shows what you picked there instead of asking a second time — two settings for one thing can disagree, and a disagreement (its Tank, your DPS) matches nothing while looking exactly like the addon being broken. Blizzard applies that filter to Dungeons only, so the addon re-checks it itself, which is what makes it work for raids too. `/alfg roles` is gone with it.
